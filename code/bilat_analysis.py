...
stakeholder_id = pd.read_excel(
    os.path.join(data_path, "stakeholder_id.xlsx"), usecols=list(range(0, 5))
)

org_id = pd.read_excel(
    os.path.join(data_path, "org_id.xlsx"), usecols=list(range(0, 6))
)

meeting_info = pd.read_excel(
    os.path.join(data_path, "meeting_info.xlsx"), usecols=list(range(0, 7)), dtype=str
)

minutes = pd.read_excel(
    os.path.join(data_path, "minutes.xlsx"), usecols=list(range(1, 9))
)

issue_id = pd.read_excel(
    os.path.join(data_path, "issue_id.xlsx"), usecols=list(range(0, 3))
)

project_id = pd.read_excel(
    os.path.join(data_path, "project_id.xlsx"), usecols=list(range(0, 3))
)
...
stakeholder_mapping = pd.Series(
    stakeholder_id["org_id"].values, index=stakeholder_id["stakeholder_id"].values
).to_dict()


# %% Process minutes
# Create a new column with a dict of org_id and count pairs
minutes["mentions"] = minutes["stakeholders_mentioned"].apply(
    count_rep, args=((stakeholder_mapping),)
)

# Convert the thai_part and chinese_party to dictionaries of org counts
meeting_info["thai_party"] = meeting_info["thai_party"].apply(
    count_rep, args=((stakeholder_mapping),)
)

meeting_info["chinese_party"] = meeting_info["chinese_party"].apply(
    count_rep, args=((stakeholder_mapping),)
)

meeting_info["other"] = meeting_info["other"].apply(
    count_rep, args=((stakeholder_mapping),)
)
...
attendees = []
for i in range(0, 27):
    attendees.append({**thai_party[i], **chinese_party[i], **lao_party[i]})
...
minutes.issue_id.replace(
    {1: "Project design", 2: "Technical", 3: "Financial", 4: "Legal"}, inplace=True
)

minutes.issue_id.value_counts().plot(
    kind="bar",
    xlabel="Issue ID",
    ylabel="Counts",
...
org_mapping = pd.Series(org_id.name.values, index=org_id.org_id.values).to_dict()
...
total_attd_count = (
    pd.Series(total_attd_count)
    .reset_index()
    .rename({"index": "org_id", 0: "attendance_count"}, axis=1)
)
...
total_attd_count["name"] = total_attd_count["org_id"].apply(
    get_org_name, args=((org_mapping),)
)

# Plot a barchart of Organization vs attendance
fig, ax = plt.subplots(figsize=(7, 5))
plt.barh(total_attd_count["name"], total_attd_count["attendance_count"])
ax.set_xlabel("Count")
...
session_mentions = pd.DataFrame(count_mentions_in_session(minutes)).fillna(0)

# Plot the result
session_mentions["total"] = session_mentions.sum(axis=1)
session_mentions["total"].plot(
    kind="bar",
    xlabel="Session",
...
abbre_mapping = pd.Series(org_id.abbre.values, index=org_id.org_id.values).to_dict()
...
plot_network(G, node_map, attendees, org_id, "Bilateral Negotiation Stage - All Actors")


# %% Compute Closeness and betweeness scores
# Create an adjacency matrix with only attendees
adjmat_attendees = create_adjmat(ids=oids, actors_count=attendees)
...
G_attendees = nx.from_numpy_array(adjmat_attendees.values)
...
btwn_score_df.sort_values("score", ascending=True, inplace=True)
btwn_score_df["org_name"] = [abbre_mapping[k] for k in btwn_score_df.index]
btwn_score_df = btwn_score_df.loc[btwn_score_df.index.isin(adjmat_attendees.index)]

clsn_score_df = pd.DataFrame(clsn_score_dict, index=["score"]).T
clsn_score_df.sort_values("score", ascending=True, inplace=True)
clsn_score_df["org_name"] = [abbre_mapping[k] for k in clsn_score_df.index]
clsn_score_df = clsn_score_df.loc[clsn_score_df.index.isin(adjmat_attendees.index)]
