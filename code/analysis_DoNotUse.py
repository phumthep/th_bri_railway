...
for i in range(0, 27):
    ...
    # The mentions might overlap with participants
    for k, v in session_mentions_dict[i].items():
        session_sth_dict[k] += v
    # The thai and chinese parties do not overlap, so we can use update
    session_sth_dict.update(thai_party[i])
    session_sth_dict.update(chinese_party[i])
    session_sth_dict.update(lao_party[i])
...
for i in range(0,27):
    oid_pairs = create_sth_pairs(stakeholders_list[i])
...
cn_attendees = []
cn_sths = []
lao_attendees = [] # there's no specific lao stakeholder mentioned
...
for i, node in enumerate(G.nodes()):
    ...
    if org_id[org_id['org_id'] == oid]['country'].values[0] == 'th':
        ...
        lao_attendees.append(node)
    
"""    node_attrs[i] = {
        'country': org_id[org_id['org_id'] == oid]['country'].values[0],
        'org_type': org_id[org_id['org_id'] == oid]['classification'].values[0]
        }
"""
pos = nx.spring_layout(G, seed=42) #seed=42
