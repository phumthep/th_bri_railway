...
rs_data_raw = pd.read_excel(os.path.join(data_path, "resolution_data.xlsx"))

senderRecipient = pd.read_excel(
    os.path.join(data_path, "resolution_sender_recipient.xlsx"),
...
senderRecipient["resolution_id"].value_counts(sort=False).plot(
    xlabel="Resolution ID", ylabel="Letter count", kind="bar"
)
plt.show()
...
rs_data.issue_id.value_counts().plot(
    kind="bar", ylabel="Counts", title="Number of key passage by category"
)
...
ax.bar(
    ...
    bottom=session_df["Project design"] + session_df["Technical"],
    label="Financial",
    color="#636363",
)
ax.bar(
...
issuesBySenders = rs_data.pivot_table(
    index="sender_abbre", columns="issue_id", aggfunc="size", fill_value=0
)
# Reorder the columns
...
ax.barh(
    ...
    left=issuesBySenders["Project design"] + issuesBySenders["Technical"],
    label="Financial",
    color="#636363",
)
ax.barh(
...
rs_data["mentions"] = rs_data["stakeholders_mentioned"].apply(count_rep_rs)

# Create a dataframe cols = org_id and rows = session_id.
# The entries represent the number of mentions
session_mentions = (
    pd.DataFrame(count_mentions_in_session(rs_data)).fillna(0).rename(abbre_mapping)
)

# Drop columns with only zeros
session_mentions = session_mentions.loc[:, (session_mentions != 0).any(axis=0)]
...
issue_mention = pd.DataFrame(
    0,
    index=["Project design", "Technical", "Financial", "Legal"],
    columns=list(stakeholders),
)
...
org_mapping = pd.Series(
    org_id.name.values, index=org_id.org_id.values.astype(str)
).to_dict()
...
ax.barh(
    ...
    left=issue_mention["Project design"] + issue_mention["Technical"],
    label="Financial",
    color="#636363",
)
ax.barh(
...
rs_adjmat = pd.DataFrame(0, index=org_id.org_id, columns=org_id.org_id)
...
G = nx.from_numpy_array(rs_adjmat.values)
...
rs_adjmat_actors = pd.DataFrame(0, index=org_id.org_id, columns=org_id.org_id)
...
# Create a graph object from adjmat
G_actors = nx.from_numpy_array(rs_adjmat_actors.values)
